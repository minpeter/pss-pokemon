...
export async function runHumanControlPlane({
  ...
  await runPokemonControlLoop({
    actor: createHumanKeyboardActor(input, traceWriter),
    backendUrl,
...
function createHumanKeyboardActor(
  input: HumanKeyInput,
  traceWriter: TraceWriter | undefined,
): PokemonControlActor {
  return {
    runTurn: async (context) => executeHumanTurn({ input, traceWriter, ...context }),
    start: () => {
      prepareInput(input)
...
async function executeHumanTurn({
  ...
  readonly input: HumanKeyInput
  readonly traceWriter: TraceWriter | undefined
  ...
  switch (decision.type) {
    case "action": {
      ...
      await recordTraceActionExecution(traceWriter, execution, turn, "human")
